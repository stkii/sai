import { Alert, Box, CloseButton, Dialog, HStack, Portal, Stack } from '@chakra-ui/react';
...
interface Props {
  onLoaded: (table: ParsedDataTable, selection: Dataset) => void;
}

export const DataImportDialog = ({ onLoaded }: Props) => {
  ...
  const handlePick = async () => {
    const result = await open({
      multiple: false,
      directory: false,
      filters: [{ name: 'DataFiles', extensions: ['xlsx', 'csv'] }],
    });
    if (!result) {
      return;
    }
  ...
  return (
    <>
      <BaseButton label="データを選択" onClick={() => setIsOpen(true)} variant="outline" />
        ...
        <Portal>
          ...
          <Dialog.Positioner>
            <Dialog.Content>
              ...
              <Dialog.Body>
                <Stack gap="3">
                  <FileSelector selectedPath={selectedPath} onPick={handlePick} />
                  {hasSheets ? (
              ...
              <Dialog.Footer>
                <HStack gap="2" justify="flex-end" w="full">
                  <BaseButton label="キャンセル" onClick={handleClose} variant="outline" />
                  <BaseButton
                    label="読み込む"
                    onClick={handleLoad}
                    loading={loadingTable}
                    disabled={!canLoad || loadingSheets}
                  />
                </HStack>
              </Dialog.Footer>
