...
5. After writing TypeScript, run `pnpm fixall`, `pnpm check`, and `pnpm tsc --noEmit` at the repo root to catch TypeScript errors; after writing Rust, run `cargo +nightly fmt` and `cargo check` in `src-tauri/` to verify formatting and errors, and fix any errors found.
